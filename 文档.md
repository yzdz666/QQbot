...
```php
// 禁言单个成员（秒数）
群禁言成员(群openid, 成员openid, 秒数)

// 解除禁言
群解禁成员(群openid, 成员openid)

// 批量禁言（自动分批，单次最多10人）
群批量禁言(群openid, 成员openid数组, 秒数)
...
// 批量禁言3人10分钟
群批量禁言(来源, ['oid1', 'oid2', 'oid3'], 600);

// 查询禁言列表
$resp = 查询群禁言状态(来源);
$data = json_decode($resp, true);
// $data['members'] 为被禁言成员列表
```

### 7.6 群信息与入群管理

```php
// 获取群基础信息（返回群名称、群简介、群分类、群标签、群成员人数）
获取群信息(群openid)
...
// 获取入群申请列表（分页）
获取入群申请列表(群openid, [游标], [每页数量])

// 处理入群申请（同意/拒绝）
处理入群申请(群openid, 成员openid, 是否同意, [拒绝原因], [加黑名单])
```

```php
// 查询群信息
$info = json_decode(获取群信息(来源), true);
文字("群名称: " . $info['group_name']);
文字("群成员数: " . $info['group_member_num']);
文字("群简介: " . $info['group_finger_memo']);
```

#### 后台自动获取群名称

框架在收到群聊消息时会自动调用 `获取群信息` API 获取群名称并存入 `groups` 表，无需手动操作。后台聊天界面会显示群名称。

后台 API 接口 `group_get_info` 支持手动刷新群信息：

```php
// POST admin/api.php action=group_get_info
// 参数: appid, group_openid
// 返回: { success, data: { group_name, group_member_num, group_finger_memo, ... } }
```
...
修改审批策略白名单('sid1', [
    'op' => 'add',
    'whitelist_users' => ['13800001111', '13800002222']
]);
```
...
修改菜单([
    'items' => [
        ['type' => 'send_message', 'label' => '签到', 'data' => '签到'],
        ['type' => 'link', 'label' => '官网', 'data' => 'https://example.com'],
    ]
]);
```
...
创建指令面板([
    ...
    'panel' => [
        'items' => [
            ...
            ['type' => 'link', 'name' => '官网', 'data' => 'https://example.com'],
        ]
    ]
]);
```
...
获取身份组列表(频道ID)
创建身份组(频道ID, 数据数组)
修改身份组(频道ID, 身份组ID, 数据数组)
删除身份组(频道ID, 身份组ID)
添加身份组成员(频道ID, 用户ID, 身份组ID)
移除身份组成员(频道ID, 用户ID, 身份组ID)
...
// 子频道权限
获取用户权限(子频道ID, 用户ID)
修改用户权限(子频道ID, 用户ID, 权限)
获取身份组权限(子频道ID, 身份组ID)
修改身份组权限(子频道ID, 身份组ID, 权限)
...
获取日程详情(子频道ID, 日程ID)
创建日程(子频道ID, 日程数据数组)
修改日程(子频道ID, 日程ID, 日程数据数组)
删除日程(子频道ID, 日程ID)

// 音频控制
音频控制(子频道ID, 状态, [音频URL], [文字])
上麦(子频道ID)
下麦(子频道ID)

// 表情表态
添加表态(子频道ID, 消息ID, 类型, 表情ID)
删除表态(子频道ID, 消息ID, 类型, 表情ID)
获取表态用户列表(子频道ID, 消息ID, 类型, 表情ID, [游标])
...
// API权限
获取API权限列表(频道ID)
申请API权限(频道ID, 子频道ID, 权限标识数组)

// 其他
获取语音成员(子频道ID)
获取在线成员数(子频道ID)
获取子频道消息(子频道ID, 消息ID)
修改子频道消息(子频道ID, 消息ID, 数据数组)
获取子频道消息列表(子频道ID, [查询参数数组])
```
...
| C2C_MSG_REJECT | 群消息拒绝 | 用户关闭主动消息推送 |
| C2C_MSG_RECEIVE | 群消息接收 | 用户开启主动消息推送 |
...
| GROUP_JOIN_REQUEST | 入群申请 | 用户申请入群 |
| GROUP_MSG_REJECT | 群消息拒绝 | 群管理员关闭群消息通知 |
| GROUP_MSG_RECEIVE | 群消息接收 | 群管理员开启群消息通知 |
| FRIEND_ADD | 好友增加 | 用户添加机器人为好友 |
| FRIEND_DEL | 好友删除 | 用户删除机器人好友 |
| SUBSCRIBED_STATUS_CHANGE | 订阅状态 | 用户订阅状态变更 |
| AT_MESSAGE_CREATE | 文字子频道 | 频道@机器人消息 |
| MESSAGE_CREATE | 文字子频道 | 频道全量消息 |
| AUDIO_OR_LIVE_CHANNEL_MEMBER_EXIT | 频道 | 音视频/直播子频道成员退出 |
| FORUM_PUBLISH_AUDIT_RESULT | 频道 | 论坛帖子审核结果 |
| GUILD_UPDATE | 频道更新 | 频道信息更新 |
| MESSAGE_REACTION_ADD | 表情表态 | 消息表情表态添加 |
| MESSAGE_REACTION_REMOVE | 表情表态 | 消息表情表态移除 |
