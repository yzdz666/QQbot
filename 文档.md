    ...
    [
        "buttons" => [
            [
                ...
                "action" => [
                    "type" => 1,  // 1=回调(点击后触发INTERACTION_CREATE), 0=跳转链接, 2=输入指令
                    "data" => "确认",  // 回调标识，点击后通过 button_data 回传
                    "enter" => false,
                    "permission" => ["type" => 2]
                ]
            ]
...
回调按钮点击后会触发 `INTERACTION_CREATE` 事件，框架自动处理 ACK 确认：
- **WS 模式**：自动通过 WebSocket 发送 `op=12` ACK
- **WH 模式**：自动在 HTTP 响应中返回 `{"op": 12, "code": 0}`

插件无需手动调用 `确认互动()`，直接处理业务逻辑即可。

```php
<?php
if (消息来源 == "互动") {
    // 消息 常量已包含 button_data (即按钮 action.data 的值)
    // 可直接用于匹配，参照 ElainaBot_v2 的处理方式
    $callbackData = 消息;
...
|------|------|-----------|
| 0 | 跳转链接 | URL |
| 1 | 回调 | 回调标识（点击后通过 button_data 回传） |
| 2 | 输入指令 | 填充到输入框的内容 |
